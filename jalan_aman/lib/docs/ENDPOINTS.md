...
```json
{
  ...
  "attachment": { "id": "uuid", "uploadUrl": "url", "s3Key": "string", "mimeType": "string", "fileSize": 0, "createdAt": "datetime" } (optional; omitted when no attachment is requested)
}
```
...
```
**Errors:** `400` (validation), `401` (unauthorized), `403` (not OP), `404` (not found), `409` (too far from report location)
...
```
**Errors:** `401` (unauthorized), `404` (not found), `409` (already voted within 24 h)
...
```
**Errors:** `400` (missing details/commentId), `401` (unauthorized or not author), `404` (not found)
...
```
**Errors:** `400` (missing commentId), `401` (unauthorized or not author), `404` (not found)
